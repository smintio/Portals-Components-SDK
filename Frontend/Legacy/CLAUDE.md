...
| `Reference/` | the sources of the components Smint.io ships — 64 UI components, 34 page templates, the shared `portals-components` library and the gallery view |
...
`Reference/` is the working source behind the last two documents. When a table does not answer
the question, read the component itself.
...
carrying the field's data type, and the free text as `query`. The canonical implementation is
in `Reference/ui-components/ui-generic-assets-preview-1` — search it for `"dt-" + id`, a typed
switch over the `ValueType` cases. Copy that, note in a comment where you copied it from so a
...
`Reference/` is an export of the sources behind the components Smint.io ships. Treat it as
read-only: edits there are overwritten by the next export. It is there to be read and copied
from.
