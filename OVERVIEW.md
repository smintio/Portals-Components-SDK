...
Please note that there is no separate *Startup* class for Smint.io Portals frontend components. Frontend components declare everything they need alongside their own source — as annotations in a *Smint.io components* (Vue 2) component, and in a `*.smintio.config.ts` configuration file in a *Sminted UI* (Vue 3) component. When a frontend component is loaded, the system generates the component's *Startup* class on the fly.

[Here](connectors/Connector-SharePoint/SharepointConnectorStartup.cs) you find an example of a component's *Startup* class (this one is part of our *Microsoft SharePoint* connector)
...
Please note that there is no separate *Configuration* class for Smint.io Portals frontend components. Frontend components declare their configurable settings alongside their own source — as annotations in a *Smint.io components* (Vue 2) component, and through the configuration DSL in a `*.smintio.config.ts` file in a *Sminted UI* (Vue 3) component. When a frontend component is loaded, the system generates the component's *Configuration* class on the fly.

[Here](connectors/Connector-Picturepark/PictureparkConnectorConfiguration.cs) you find an example of a component's *Configuration* class (this one is part of our *Picturepark connector)
...
Smint.io Portals frontend components come in two generations:

- **[Sminted UI](sminted-ui/)** — the current generation, built with Vue 3. Start here for all new work.
- **[Smint.io components](smintio-components/)** — the previous generation, built with Vue 2. Still
  supported, and still the most complete description of what each component type is expected to do.

Both generations use the same component type ids, the same page type contracts and the same data
adapter model. Only the authoring technology differs.
